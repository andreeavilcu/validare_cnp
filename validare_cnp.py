...
COUNTIES = {
    ...
    "40": "Bucharest",      "41": "Bucharest S1",    "42": "Bucharest S2",
    "43": "Bucharest S3",   "44": "Bucharest S4",    "45": "Bucharest S5",
    "46": "Bucharest S6",   "51": "Calarasi",        "52": "Giurgiu",
}
...
def validate_cnp(raw):
    ...
    month = ll
    day   = zz
    year  = 1900 + aa if s in (1, 2) else 2000 + aa
    ...
    if jj not in COUNTIES:
        return False, f"Invalid CNP: county code ({jj}) is not a valid Romanian county code.", None
...
def on_verify():
    ...
    if valid:
        result_label.config(text="[OK] " + message, fg="#1B5E20", bg="#E8F5E9")
        info_label.config(text="\n".join(f"{k}: {v}" for k, v in info.items()))
    else:
        result_label.config(text="[X] " + message, fg="#B71C1C", bg="#FFEBEE")
        info_label.config(text="")
